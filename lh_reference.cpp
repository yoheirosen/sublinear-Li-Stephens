...
size_t linearReferenceStructure::absolute_length() {
  return site_index_to_position.back() + span_lengths.back();
}
  ...
  for(size_t i = 0; i < length_of_haplotypes; i++) {
    ...
    for(size_t j = 0; j < number_of_haplotypes; j++) {
      for(size_t k = 0; k < alleles_seen.size(); k++) {
        if(char_to_allele(haplotypes[i][j], ref_at_i) != alleles_seen[k]) {
          alleles_seen.push_back(char_to_allele(haplotypes[i][j], ref_at_i));
        }
      }
    }
...
alleleValue haplotypeCohort::allele_at(size_t site_index, size_t haplotype_index) {
  return haplotype_alleles_by_site_index[site_index][haplotype_index];
}
...
void haplotypeCohort::populate_allele_counts() {
  for(size_t i = 0; i < haplotype_alleles_by_site_index.size(); i++) {
    for(int j = 0; j < 5; j++) {
      int allele_rank = (int)haplotype_alleles_by_site_index[i][j];
      allele_counts_by_site_index[i][allele_rank]++;
    }
  }
}
...
vector<size_t> haplotypeCohort::get_matches(size_t site_index, alleleValue a) {
  ...
  for(size_t i = 0; i < number_of_haplotypes; i++) {
    if(haplotype_alleles_by_site_index[site_index][i] == a) {
...
void linearReferenceStructure::add_site(size_t position, 
  ...
  if(new_index == 0) {
    leading_span_length = position - 1;
  } else {
...
haplotypeCohort::haplotypeCohort(vector<string>& haplotypes, 
  ...
  for(size_t i = 0; i < reference->number_of_sites(); i++) {
    haplotype_alleles_by_site_index.push_back(
                vector<alleleValue>(number_of_haplotypes));
    size_t j = reference->get_position(i);
    for(size_t k = 0; k < number_of_haplotypes; k++) {
      haplotype_alleles_by_site_index[i][k] = char_to_allele(haplotypes[i][j],
