...
int main() {
  ...
  vector<size_t> populations = {5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000, 25000};
...
//   for(int n = 0; n < allele_frequencies.size(); n++) {
//     bernoulli_distribution bernoulli(allele_frequencies[n]);
//     string ref_refstring;
//     string read_refstring;
//     
//     vector<bool> shared_sites = vector<bool>(lengths[k], false);
//     vector<alleleValue> read_site_values = 
//     bool flip_site;
//     // set shared sites
...
//     
//     
//     vector<size_t> cutoffs = {-8, -13, -20, -27, -34};
//     for(int m = 0; m < 10; m++) {
//       for(int l = 0; l < populations.size(); l++) {
//         // lengths
//         for(int k = 0; k < lengths.size(); k++) {
...
//           auto end = chrono::high_resolution_clock::now();
//           auto ms = chrono::duration_cast<chrono::milliseconds>(end - begin).count();
//           cout << "frequency\t" << allele_frequencies[n] << "\tbp\t" << total_length << "\t sites \t" << lengths[k] << "\t population size \t" << populations[l] << "\t time \t" << ms << "\t ms\t" << minor_allele_count << "\t minor allele count \t" << hap_manager.get_current_leaves().size() "\t terminal leaves" << endl;
//         }
//       }
//     }
//   }
